...
<template>
    <v-card variant="flat" class="rounded-md card-hover overflow-hidden bg-surface">
        ...
        <div class="d-flex justify-end mr-3 mt-n6 position-absolute action-btn">
            <v-btn icon v-bind="props" color="white" size="small" class="mr-3"><HeartIcon size="19" stroke-width="1.5" /></v-btn>
            <v-tooltip text="Add to Cart" location="bottom">
                <template v-slot:activator="{ props }">
                    <v-btn icon v-bind="props" color="white" size="small" @click="$emit('handlecart', (successsnackbar = true))"
        ...
        <v-card-item class="pa-0 pt-1 text-left">
            ...
            <div class="d-flex align-center justify-start mt-2">
                ...
                ><span class="font-weight-semibold text-grey200 text-subtitle-1">{{ rating?.toFixed(1) }}</span>
